...
// from Waveshare. Requires HW SPI and Adafruit_GFX. Caution: these e-papers
...
//
// Modified by likeablob for SMP7C. To have single paged operation.
...
// Modify to have a pseudo page buffer which is actually splitted into two byte
// buffers. This is because ESP32 doesn't allow allocating 131 KiB(600 * 448 /
// 2) of a single contiguous buffer.
#define PIXEL_BUFFER_LEN ((GxEPD2_Type::WIDTH / 2) * page_height / 2)
#define PIXEL_BUFFER(i)                                                        \
    ((i) < PIXEL_BUFFER_LEN ? (_pixel_buffer_a[(i)])                           \
                            : (_pixel_buffer_b[(i)-PIXEL_BUFFER_LEN]))

// Hidden 8th color?
#define GxEPD_BEIGE 0x07

#include "GxEPD2_565c_mod.h"
...
class GxEPD2_7C : public GxEPD2_GFX_BASE_CLASS {
  public:
    ...
    GxEPD2_7C(GxEPD2_Type epd2_instance)
        : GxEPD2_GFX_BASE_CLASS(epd2, GxEPD2_Type::WIDTH, GxEPD2_Type::HEIGHT),
          epd2(epd2_instance)
#else
    GxEPD2_7C(GxEPD2_Type epd2_instance)
        : GxEPD2_GFX_BASE_CLASS(GxEPD2_Type::WIDTH, GxEPD2_Type::HEIGHT),
          epd2(epd2_instance)
#endif
    {
    ...
    void drawPixel(int16_t x, int16_t y, uint16_t color) {
        ...
        if(x & 1)
            // _pixel_buffer_a[i] = (_pixel_buffer_a[i] & 0xF0) | pv;
            PIXEL_BUFFER(i) = (PIXEL_BUFFER(i) & 0xF0) | pv;
        else
            // _pixel_buffer_a[i] = (_pixel_buffer_a[i] & 0x0F) | (pv << 4);
            PIXEL_BUFFER(i) = (PIXEL_BUFFER(i) & 0x0F) | (pv << 4);
    }
    ...
    void init(uint32_t serial_diag_bitrate = 0) // = 0 : disabled
    {
        ...
        _using_partial_mode = false;
        _current_page = 0;
        _pixel_buffer_a = (uint8_t *)malloc(PIXEL_BUFFER_LEN);
        _pixel_buffer_b = (uint8_t *)malloc(PIXEL_BUFFER_LEN);
        setFullWindow();
    }

    // init method with additional parameters:
    // initial false for re-init after processor deep sleep wake up, if display
    // power supply was kept only relevant for b/w displays with fast partial
    // update reset_duration = 20 is default; a value of 2 may help with
    ...
    void init(uint32_t serial_diag_bitrate, bool initial,
              uint16_t reset_duration = 20, bool pulldown_rst_mode = false) {
    ...
    void fillScreen(uint16_t color) {
        uint8_t pv = color7(color);
        uint8_t pv2 = pv | pv << 4;
        for(uint32_t x = 0; x < PIXEL_BUFFER_LEN; x++) {
            _pixel_buffer_a[x] = pv2;
        }
        for(uint32_t x = 0; x < PIXEL_BUFFER_LEN; x++) {
            _pixel_buffer_b[x] = pv2;
        }
    }

    // display buffer content to screen, useful for full screen buffer
    void display(bool partial_update_mode = false) {
        epd2.writeNative(_pixel_buffer_a, 0, 0, 0, WIDTH, _page_height / 2);
        epd2.writeNative(_pixel_buffer_b, 0, 0, _page_height / 2, WIDTH,
                         _page_height / 2);
    ...
    void displayWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
        ...
        _rotate(x, y, w, h);
        epd2.writeNativePart(_pixel_buffer_a, 0, x, y, WIDTH, _page_height / 2,
                             x, y, w, h);
    ...
    bool nextPage() {
        ...
        if(_using_partial_mode) {
            ...
            uint16_t page_ye = _current_page < int16_t(_pages - 1)
                                   ? page_ys + _page_height
                                   : HEIGHT;
            uint16_t dest_ys = _pw_y + page_ys; // transposed
            uint16_t dest_ye = gx_uint16_min(_pw_y + _pw_h, _pw_y + page_ye);
            if(dest_ye > dest_ys) {
                // Serial.print("writeImage("); Serial.print(_pw_x);
                // Serial.print(", "); Serial.print(dest_ys); Serial.print(",
                // "); Serial.print(_pw_w); Serial.print(", ");
                // Serial.print(dest_ye - dest_ys); Serial.println(")");
                epd2.writeNative(_pixel_buffer_a, 0, _pw_x, dest_ys, _pw_w,
                                 dest_ye - dest_ys);
            } else {
                // Serial.print("writeImage("); Serial.print(_pw_x);
                // Serial.print(", "); Serial.print(dest_ys); Serial.print(",
                // "); Serial.print(_pw_w); Serial.print(", ");
                // Serial.print(dest_ye - dest_ys); Serial.print(") skipped ");
                // Serial.print(dest_ys); Serial.print("..");
                // Serial.println(dest_ye);
            }
            _current_page++;
        ...
        {
            epd2.writeNative(_pixel_buffer_a, 0, 0, 0, WIDTH, page_height / 2);
            epd2.writeNative(_pixel_buffer_b, 0, 0, page_height / 2, WIDTH,
                             page_height / 2);
            _current_page++;
            if(_current_page == int16_t(_pages)) {
                _current_page = 0;
    ...
    void drawPaged(void (*drawCallback)(const void *), const void *pv) {
        if(_using_partial_mode) {
            for(_current_page = 0; _current_page < _pages; _current_page++) {
                uint16_t page_ys = _current_page * _page_height;
                uint16_t page_ye = _current_page < (_pages - 1)
                                       ? page_ys + _page_height
                                       : HEIGHT;
                uint16_t dest_ys = _pw_y + page_ys; // transposed
                uint16_t dest_ye =
                    gx_uint16_min(_pw_y + _pw_h, _pw_y + page_ye);
                if(dest_ye > dest_ys) {
                    fillScreen(GxEPD_WHITE);
                    drawCallback(pv);
                    epd2.writeNative(_pixel_buffer_a, 0, _pw_x, dest_ys, _pw_w,
        ...
        } else // full update
        {
            ...
            for(_current_page = 0; _current_page < _pages; _current_page++) {
                ...
                fillScreen(GxEPD_WHITE);
                drawCallback(pv);
                epd2.writeNative(_pixel_buffer_a, 0, 0, 0, WIDTH,
                                 page_height / 2);
                epd2.writeNative(_pixel_buffer_b, 0, 0, page_height / 2, WIDTH,
                                 page_height / 2);
            }
    ...
    void drawImagePart(const uint8_t bitmap[], int16_t x_part, int16_t y_part,
                       ...
                       bool mirror_y = false, bool pgm = false) {
        epd2.drawImagePart(bitmap, x_part, y_part, w_bitmap, h_bitmap, x, y, w,
                           h, invert, mirror_y, pgm);
    }
    ...
    void drawImagePart(const uint8_t *black, const uint8_t *color,
                       ...
                       int16_t h_bitmap, int16_t x, int16_t y, int16_t w,
                       int16_t h, bool invert, bool mirror_y, bool pgm) {
        epd2.drawImagePart(black, color, x_part, y_part, w_bitmap, h_bitmap, x,
                           y, w, h, invert, mirror_y, pgm);
    }
    void drawImagePart(const uint8_t *black, const uint8_t *color,
                       int16_t x_part, int16_t y_part, int16_t w_bitmap,
    ...
    uint8_t color7(uint16_t color) {
        ...
        default: {
            ...
            if((red < 0x8000) && (green < 0x8000) && (blue < 0x8000))
                ...
                static const uint16_t y2o_lim =
                    ((GxEPD_YELLOW - GxEPD_ORANGE) / 2 +
                     (GxEPD_ORANGE & 0x07E0))
                    << 5;
  ...
  private:
    ...
    // uint8_t _pixel_buffer_a[PIXEL_BUFFER_LEN];
    // uint8_t _pixel_buffer_a[(GxEPD2_Type::WIDTH / 2) * page_height / 2];
    // uint8_t _pixel_buffer_b[(GxEPD2_Type::WIDTH / 2) * page_height / 2];
    uint8_t *_pixel_buffer_a;
    uint8_t *_pixel_buffer_b;
