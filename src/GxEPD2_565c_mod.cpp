...
// http://www.e-paper-display.com/download_detail/downloadsId=808.html
// Panel: 5.65inch ACeP 7-Color E-Paper :
// https://www.waveshare.com/product/displays/e-paper/5.65inch-e-paper-module-f.htm
// Controller: unknonw
// initcode extracted from Waveshare library file epd5in65f.cpp available here:
...
//
// Modified by likeablob for SMP7C. Removed busy wait handling.

#include "GxEPD2_565c_mod.h"

GxEPD2_565c::GxEPD2_565c(int16_t cs, int16_t dc, int16_t rst, int16_t busy)
    : GxEPD2_EPD(cs, dc, rst, busy, LOW, 25000000, WIDTH, HEIGHT, panel,
                 hasColor, hasPartialUpdate, hasFastPartialUpdate) {
    _paged = false;
}

void GxEPD2_565c::clearScreen(uint8_t value) { clearScreen(value, 0xFF); }
...
void GxEPD2_565c::writeImage(const uint8_t bitmap[], int16_t x, int16_t y,
    ...
    if(_paged && (x == 0) && (w == int16_t(WIDTH)) && (h < int16_t(HEIGHT))) {
        ...
        for(int16_t i = 0; i < int16_t(HEIGHT); i++) {
            for(int16_t j = 0; j < int16_t(WIDTH); j += 8) {
                ...
                for(int16_t k = 0; k < 4; k++) {
                    uint8_t data2 = (data & 0x80 ? 0x10 : 0x00) |
                                    (data & 0x40 ? 0x01 : 0x00);
                    data <<= 2;
                    _transfer(data2);
                }
...
void GxEPD2_565c::writeImage(const uint8_t *black, const uint8_t *color,
...
void GxEPD2_565c::writeImagePart(const uint8_t bitmap[], int16_t x_part,
        ...
        for(int16_t j = 0; j < int16_t(WIDTH); j += 8) {
            ...
            if((j >= x1) && (j < x1 + w) && (i >= y1) && (i < y1 + h)) {
                ...
                uint32_t idx =
                    mirror_y
                        ? (x_part + j - x1) / 8 +
                              uint32_t((h_bitmap - 1 - (y_part + i - y1))) *
                                  wb_bitmap
                        : (x_part + j - x1) / 8 +
...
void GxEPD2_565c::writeImagePart(const uint8_t *black, const uint8_t *color,
    ...
    if(!black && !color)
        return;
    if(!color)
        return writeImagePart(black, x_part, y_part, w_bitmap, h_bitmap, x, y,
...
void GxEPD2_565c::writeNative(const uint8_t *data1, const uint8_t *data2,
                              int16_t x, int16_t y, int16_t w, int16_t h,
                              bool invert, bool mirror_y, bool pgm) {
    if(data1) {
...
void GxEPD2_565c::writeNativePart(const uint8_t *data1, const uint8_t *data2,
                                  int16_t x_part, int16_t y_part,
                                  int16_t w_bitmap, int16_t h_bitmap, int16_t x,
                                  int16_t y, int16_t w, int16_t h, bool invert,
                                  bool mirror_y, bool pgm) {
...
void GxEPD2_565c::drawImage(const uint8_t bitmap[], int16_t x, int16_t y,
                            int16_t w, int16_t h, bool invert, bool mirror_y,
                            bool pgm) {
...
void GxEPD2_565c::drawImagePart(const uint8_t bitmap[], int16_t x_part,
                                int16_t y_part, int16_t w_bitmap,
                                int16_t h_bitmap, int16_t x, int16_t y,
                                int16_t w, int16_t h, bool invert,
                                bool mirror_y, bool pgm) {
...
void GxEPD2_565c::drawImage(const uint8_t *black, const uint8_t *color,
                            int16_t x, int16_t y, int16_t w, int16_t h,
                            bool invert, bool mirror_y, bool pgm) {
...
void GxEPD2_565c::drawImagePart(const uint8_t *black, const uint8_t *color,
                                int16_t x_part, int16_t y_part,
                                int16_t w_bitmap, int16_t h_bitmap, int16_t x,
                                int16_t y, int16_t w, int16_t h, bool invert,
                                bool mirror_y, bool pgm) {
...
void GxEPD2_565c::drawNative(const uint8_t *data1, const uint8_t *data2,
                             int16_t x, int16_t y, int16_t w, int16_t h,
                             bool invert, bool mirror_y, bool pgm) {
...
void GxEPD2_565c::powerOff() { _PowerOff(); }
